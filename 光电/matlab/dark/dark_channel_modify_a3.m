...
%改进透射率，联合双边滤波
gray = rgb2gray(imageres);
image_tran_estimate_temp=image_tran_estimate;

image_tran_estimate = jbfltGray(double(gray)/255,image_tran_estimate_temp,1,3,9);
...
imwrite(imagedes,'imagedes1_de.jpg');
figure,imshow(imagedes,[0 255]);

%% 处理
imagedes(:,:,1)=histeq(imagedes(:,:,1));%histeq,adapthisteq
imagedes(:,:,2)=histeq(imagedes(:,:,2));
imagedes(:,:,3)=histeq(imagedes(:,:,3));
figure,imshow(imagedes,[0 255]);
